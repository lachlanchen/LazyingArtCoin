(() => {
    ...
    const CHAIN_FALLBACK_NAME = "Sepolia Testnet";
    let artifact;
    let walletProvider;
    let signer;
    let walletAddress;
    ...
    const STORAGE_KEYS = {
        contract: "lac_test:contractAddress",
        owner: "lac_test:ownerAddress",
        treasury: "lac_test:treasuryAddress",
        supply: "lac_test:initialSupply"
    };
    ...
    const updateNetworkStatus = (chainId) => {
        ...
        const name = chainId === DESIRED_CHAIN_ID ? CHAIN_FALLBACK_NAME : `Unknown (${chainId})`;
    ...
    const ensureWallet = async () => {
        ...
        if (Number(chainId) !== DESIRED_CHAIN_ID) {
            await switchNetwork(DESIRED_CHAIN_ID);
            const updated = await walletProvider.getNetwork();
            updateNetworkStatus(Number(updated.chainId));
        }

        return { signer, chainId: DESIRED_CHAIN_ID };
    };
    ...
    const handleDeploy = async (evt) => {
        ...
        elements.deployOutput.textContent = "Deploying LazyArtCoin (test)… confirm the transaction in MetaMask.";
    ...
    const getActiveContract = (requireSigner = true) => {
        ...
        const provider = requireSigner ? signer : walletProvider || ethers.getDefaultProvider("sepolia");
    ...
    const refreshStats = async () => {
        try {
            ...
            const provider = walletProvider || ethers.getDefaultProvider("sepolia");
    ...
    const init = async () => {
        updateNetworkStatus(DESIRED_CHAIN_ID);
        loadFormDefaults();
        try {
            await loadArtifact();
        } catch (err) {
